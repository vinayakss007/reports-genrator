...
import type * as echartsT from "echarts";
...
import { SaveAsTile } from "../dashboards/SaveAsTile.js";
import type {
  ChartSpec,
  ComputedChart,
  Profile,
...
export function BuildPage() {
  ...
  const echartsRef = useRef<echartsT.ECharts | null>(null);
  ...
  const seriesValues = useMemo<{ values: number[]; labels?: string[] } | null>(() => {
    ...
    const yName =
      ySlot.label ??
      (ySlot.agg && ySlot.field !== "*"
        ? `${ySlot.agg}_${ySlot.field}`
        : ySlot.agg === "count"
          ? "count"
          : ySlot.field);
  ...
  async function handlePgFlow() {
    await runFlow(async () => {
      ...
      const dsRes = await fetch("/api/datasets", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ sourceId: src.id, name: "query", query: pg.query }),
      });
      if (!dsRes.ok) throw new Error(`dataset failed: HTTP ${dsRes.status}`);
      const ds = (await dsRes.json()) as { id: string };
      await previewRecommendEncode(ds.id);
  ...
  async function previewRecommendEncode(id: string) {
    setDatasetId(id);
    const pvRes = await fetch(`/api/datasets/${id}/preview`, {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    <div>
      ...
      <fieldset style={{ padding: "1rem" }}>
        ...
        <label style={{ marginRight: "1rem" }}>
          <input type="radio" checked={mode === "file"} onChange={() => setMode("file")} /> File (CSV/XLSX)
        </label>
        <label>
          <input type="radio" checked={mode === "postgres"} onChange={() => setMode("postgres")} /> Postgres
        </label>
        {mode === "file" ? (
          <div style={{ marginTop: "0.75rem" }}>
            <input type="file" accept=".csv,.xlsx" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
            <button style={{ marginLeft: "1rem" }} disabled={busy || !file} onClick={() => void handleFileFlow()}>
              {busy ? "Working..." : "Upload & Recommend"}
            </button>
          </div>
        ) : (
          <PgFormView pg={pg} setPg={setPg} busy={busy} onSubmit={() => void handlePgFlow()} />
        )}
        {error && <p style={{ color: "crimson" }}>Error: {error}</p>}
      </fieldset>

      {preview && recommendation && spec && datasetId && (
        <section style={{ marginTop: "1.5rem" }}>
          ...
          <div style={{ marginTop: "1rem", display: "flex", gap: "0.5rem", alignItems: "center" }}>
            <ExportButtons
              serverTarget={{ kind: "chart", datasetId, spec }}
              getEChartsInstance={() => echartsRef.current}
              filenameStem={`${spec.chart}_chart`}
            />
            <SaveAsTile
              datasetId={datasetId}
              spec={spec}
              tileTitle={spec.chart}
              onSaved={() => alert("Saved. See the Dashboards tab.")}
            />
          </div>

          <div style={{ marginTop: "0.75rem", border: "1px solid #e5e7eb", padding: "0.5rem", borderRadius: 6 }}>
            {computed ? (
              <ChartRenderer
                computed={computed}
